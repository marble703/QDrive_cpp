...
Interface::Interface(std::shared_ptr<qdriver::io::Serial> serialPort):
...
ioType Interface::getIoType(std::shared_ptr<std::string> ioTypeName) const {
    if (ioTypeName) {
        switch (this->ioType_) {
            case ioType::SERIAL:
                *ioTypeName = "SERIAL";
...
bool Interface::sendCommand(const Command& command) {
    switch (this->ioType_) {
        case ioType::SERIAL:
...
bool Interface::startReaderThread(std::function<void(std::string&)> readerFunction) {
...
bool Interface::isSerialPortOpen() const {
